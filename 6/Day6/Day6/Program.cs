...
I)SAN", 4);
        ...
        {
            Node<string> root = Parse(input);
            Debug.Assert(root.Object == expectedRoot);

            Debug.Assert(ComputeOrbits(root) == expectedOrbits);
        }

        private static Node<string> Parse(string input)
        {
            ...
            foreach (string line in input.Split(new[] { '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                {
                    EnsureExists(nodes, o);
                }

                nodes[objects[0]].Nodes.Add(nodes[objects[1]]);
                nodes[objects[1]].Parent = nodes[objects[0]];
            }

            return nodes.Values.First(x => x.Parent == null);
        }

        private static void EnsureExists(Dictionary<string, Node<string>> nodes, string obj)
        {
            if (!nodes.TryGetValue(obj, out _))
            {
                nodes.Add(obj, new Node<string>(obj));
            }
        }

        private static int ComputeOrbits<T>(Node<T> root)
        {
            Dictionary<Node<T>, int> orbits = new Dictionary<Node<T>, int>()
            {
                { root, 0 }
            };

            ComputeOrbits(orbits, root);

            return orbits.Values.Sum();
        }

        private static void ComputeOrbits<T>(Dictionary<Node<T>, int> orbits, Node<T> node)
        {
            int count = 0;
            foreach (var current in node.Nodes)
            {
                if (!orbits.TryGetValue(current, out var orbitCount))
                {
                    ComputeOrbits(orbits, current);

                    orbitCount = orbits[current];
                }

...
namespace Day9
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            VerifyIntCode("3,0,4,0,99", new long[] { 5 }, new long[] { 5 });
            VerifyIntCode("1101,100,-1,4,0", Array.Empty<long>(), Array.Empty<long>(), new (long, long)[] { (4, 99) });
            VerifyIntCode("1002,4,3,4,33", Array.Empty<long>(), Array.Empty<long>(), new (long, long)[] { (4, 99) });

            VerifyOutput("109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99", x => Parse("109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99").SequenceEqual(x));
            VerifyOutput("1102,34915192,34915192,7,4,7,99,0", x => x[0].ToString().Length == 16);
            VerifyOutput("104,1125899906842624,99", x => x.Count == 1 && x[0] == 1125899906842624);
        ...
        {
            System.Collections.IEnumerator inputStream = input.GetEnumerator();

            List<long> output = new List<long>();
            Memory memory = new Memory(Parse(inputCode));

            VM vm = new VM(memory, () => { inputStream.MoveNext(); return (long)inputStream.Current; }, x => output.Add(x), debug: false);
        ...
        private static string Stringify(List<long> result)
        {
            return string.Join(",", result);
        }

        private static List<long> IntCode(List<long> input)
        {
            Memory memory = new Memory(input);

            VM vm = new VM(memory, null, null);
        ...
        private static long GetKeycode(string input, long inputValue)
        {
            List<long> data = Parse(input);
        ...
        private static void VerifyOutput(string input, Predicate<List<long>> expectedOutput)
        {
            List<long> data = Parse(input);

            List<long> output = new List<long>();

            VM vm = new VM(new Memory(data), null, x => output.Add(x), false);
            vm.Decompile();
            ...
            Debug.Assert(expectedOutput(output));
        }
